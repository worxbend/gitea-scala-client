...
- `GiteaResponseMapper` decodes successful JSON responses, non-paginated JSON arrays, paginated issue/repository/branch/tag/release/pull-request/reaction/notification lists, object-shaped user-search and topic-name pages, repository pin-capacity responses, 204/unit responses, Gitea error payloads, raw failure bodies, pagination headers, and rate-limit reset headers.
...
- `PullRequestsApi` is mixed into `GiteaClient` with unambiguous `client.pullRequests(owner, repo, params)`, `client.pinnedPullRequests(owner, repo)`, and `client.pullRequest(owner, repo, index)` facade methods.
- Pull request list responses decode through the existing paginated JSON mapper as `#/responses/PullRequestList`; pinned pull-request lists decode as non-paginated chunks from `#/responses/PullRequestList`; single pull request responses decode as `#/responses/PullRequest`.
...
- `GiteaClientSpec` also covers multi-page pull request streaming, pinned pull-request listing, and single-pull-request lookup through a `BackendStub[Task]`.
...
- likely continue with the next pull-request read slice from `plugin-redoc-2.yaml`, such as `GET /repos/{owner}/{repo}/pulls/{base}/{head}` (`repoGetPullRequestByBaseHead`), or move to the next small repository-management endpoint,
