...
- first add a lightweight spec-trace audit for the currently hand-written endpoint metadata: compare `GiteaEndpoints` operation IDs, methods, paths, required parameters, and response labels against `plugin-redoc-2.yaml` for the implemented surface, starting with the pull-request review/comment lifecycle endpoints,
- then return to the next typed API slice now that the release infrastructure has CI, Central publishing groundwork, Renovate tracking, compatibility snapshots, issue write endpoints, and pull-request review lifecycle endpoints,
- likely continue with the next typed slice from `plugin-redoc-2.yaml`, prioritizing commit-status endpoints or pull-request merge/write operations because they add high-value workflows and will exercise request bodies, status-specific response handling, and error mappings beyond the current review lifecycle commands,
- keep no-body lifecycle commands covered with explicit request-body, content-type, retryability, and documented 400/403/404 failure assertions,
