...
- `ReleasesApi` is mixed into `GiteaClient` with unambiguous `client.releases(owner, repo, params = ReleaseListParams.default)`, `client.release(owner, repo, id)`, `client.releaseByTag(owner, repo, tag)`, `client.releaseAssets(owner, repo, releaseId)`, and `client.releaseAsset(owner, repo, releaseId, assetId)` facade methods.
...
- Documentation alignment for the release-list filtering slice and asset-list probe improvement is complete in `README.md`, `CHANGELOG.md`, and this plan. Fresh senior review found no functional blocker in the request-layer or facade path, but identified one API-shape gap: `ReleaseListParams.page` is honored by the low-level `GiteaRequests.repoReleases` builder, while the high-level streaming facade overwrites it through `Pagination.paginated`; README examples and future facade tests should avoid implying caller-controlled start-page semantics unless that behavior is deliberately added. Focused review validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; the integration run reported all live probes ignored without credentials.

## Next Continuation

- Before adding more release endpoints, reconcile release stream pagination semantics. Either document and test that `ReleaseListParams.page` is a low-level request-builder control while `ReleasesApi.releases` always starts at page 1, or add an explicit stream start-page behavior that preserves caller intent. Update the README example to avoid passing `page = Some(1)` as if it controlled the streaming facade unless that semantic is implemented.
- Next low-risk implementation continuation after that cleanup: add the read-only latest-release endpoint `GET /repos/{owner}/{repo}/releases/latest` (`repoGetLatestRelease`). It should reuse the existing `Release` model and single-release response decoding, add `GiteaRequests.repoLatestRelease`, expose a clear `ReleasesApi.latestRelease(owner, repo)` facade method, preserve read-only retry eligibility, propagate documented 404 failures, and add request/facade/retry tests plus Swagger-backed endpoint audit coverage for method, path, required `owner`/`repo` path parameters, success response ref, documented non-2xx labels, no query parameters, and no request body.
- Extend the release metadata live confidence suite only after `latestRelease` lands: add an optional `GITEA_LATEST_RELEASE_TAG` assertion or compare the returned release to `GITEA_RELEASE_TAG` only when the configured repository state makes that relationship explicit. Do not infer latest-release correctness from arbitrary release IDs.
