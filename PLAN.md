...
The next continuation can proceed to the adjacent read-only release lookup by tag endpoint before any release write or upload surface:

- add `GET /repos/{owner}/{repo}/releases/tags/{tag}` (`repoGetReleaseByTag`) from `plugin-redoc-2.yaml`, using the existing `Release` model and response mapper;
- expose a facade name that does not collide with `release(owner, repo, id)`, such as `releaseByTag(owner, repo, tag)`;
- encode slash-containing or punctuation-heavy tag names as one path segment and add request tests for values such as `v1.0.0` and `release/candidate`;
- keep the endpoint read-only retryable, with no query parameters, no request body, JSON accept headers, documented 404 propagation, and private audit-only non-2xx expectations;
- add request-layer, endpoint-audit, facade, retry, docs, changelog, and public API snapshot updates for the release-by-tag slice;
- do not add release create/edit/delete, release asset upload/edit/delete, or binary asset download APIs in that continuation;
- optionally add a hermetic live release metadata probe later, gated on explicit `GITEA_RELEASE_ID` and `GITEA_RELEASE_ASSET_ID`, but do not make it part of the release-by-tag critical path unless live confidence is needed for a real instance;
