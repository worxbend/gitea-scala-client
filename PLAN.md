...
- Documented non-2xx response status/ref label checks now cover all currently audited endpoint groups: pull-request review lifecycle, commit-status, and pull-request merge/update.
...
- implement the next Swagger-backed pull-request write slice for `POST /repos/{owner}/{repo}/pulls` (`repoCreatePullRequest`) and `PATCH /repos/{owner}/{repo}/pulls/{index}` (`repoEditPullRequest`) with typed `CreatePullRequestOption` and `EditPullRequestOption` models traced to `plugin-redoc-2.yaml`,
- add request-layer tests for path encoding, JSON body construction, `Content-Type`, success decoding, non-retryable write semantics, and documented failure mapping for create/edit, including `403`, `404`, `409`, `422`, create's documented `423`, and edit's documented `412`,
- add endpoint metadata audit coverage for the create/edit slice at the same time as the request builders: operation ID, method, path, required path parameters, success response labels, request-body presence, retryability, and documented non-2xx response status/ref labels,
