...
- `ReleasesApi` is mixed into `GiteaClient` with unambiguous `client.releases(owner, repo)`, `client.release(owner, repo, id)`, `client.releaseAssets(owner, repo, releaseId)`, and `client.releaseAsset(owner, repo, releaseId, assetId)` facade methods.
...
- Release list responses decode through the existing paginated JSON mapper as `#/responses/ReleaseList`; single release responses decode as `#/responses/Release`; release asset lists decode as non-paginated chunks from `#/responses/AttachmentList`; single release assets decode as `#/responses/Attachment`.
...
- `GiteaClientSpec` also covers multi-page release streaming, single-release lookup, non-paginated release asset listing, single release asset lookup, documented release asset not-found propagation, and release asset read-only retry behavior through a `BackendStub[Task]`.
...
- Release confidence hardening is complete for the existing read-only release metadata surface. `GiteaEndpointAuditSpec` now registers `repoListReleases` and `repoGetRelease` alongside the existing release-by-tag and release asset metadata audits, including operation IDs, methods, paths, required path parameters, release-list optional query names (`draft`, `pre-release`, `page`, `limit`), success response refs, documented non-2xx labels, request-body absence, and read-only retryability.
- Read-only release live probes are complete in `LiveGiteaIntegrationSpec` for `client.release(owner, repo, id)`, `client.releaseByTag(owner, repo, tag)`, `client.releaseAssets(owner, repo, releaseId)`, and `client.releaseAsset(owner, repo, releaseId, assetId)`. The probes are gated by non-empty `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, plus `GITEA_RELEASE_ID`, `GITEA_RELEASE_TAG`, and/or `GITEA_RELEASE_ASSET_ID` as appropriate, so credential-free integration runs remain hermetic.
- README and CHANGELOG now document the release audit scope literally as release list/detail, release-by-tag, and release asset metadata endpoints; README also documents `GITEA_RELEASE_ID`, `GITEA_RELEASE_TAG`, and `GITEA_RELEASE_ASSET_ID`, and the credential-stripped `it.test` command unsets those release variables with the rest of the live-probe set.
- Validation for the release confidence continuation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; the integration run reported all 11 live probes as ignored without credentials.
- This continuation deliberately adds validation confidence only. Release create/edit/delete, release asset upload/edit/delete, and release asset binary download APIs remain absent; `repoDeleteReleaseByTag` remains out of scope even though it shares the `/releases/tags/{tag}` route shape.
- Before implementing attachment upload/edit/delete for releases, issues, or comments, decide the shared attachment model boundary and multipart/form-data byte-upload semantics explicitly rather than widening `ReleaseAsset` by accident.
- Keep any future Swagger array query parameter that uses `collectionFormat: multi` represented and tested with ordered repeated query pairs, not `paramsMap`, so duplicate keys remain observable.
- Keep `ReposApi.rawFile`, `ReposApi.mediaFile`, and `ReposApi.archive` as buffered `Chunk[Byte]` APIs for now; defer streaming download abstractions to a deliberate large-binary slice.
...
- Fresh senior review of the release confidence slice found no functional blocker. Focused validation passed with `git diff --check`, `./mill --no-server core.test`, `./mill --no-server client.test`, `./mill --no-server compatibility.check`, `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaEndpointAuditSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; the integration run reported all 11 live probes as ignored without credentials.
- Highest-priority next implementation continuation: add `ReleaseListParams` for the already-audited `repoListReleases` optional query controls. It should model `draft`, `pre-release`, `page`, and `limit`, keep absent filters omitted by default, preserve the `pre-release` wire name, flow through `GiteaRequests.repoReleases`, `ReleasesApi.releases(owner, repo, params = ReleaseListParams.default)`, and `SttpGiteaClient`, and keep the existing `client.releases(owner, repo)` call source-compatible through a default argument.
- The release-list params slice should add request/facade tests for explicit `draft = true/false`, `pre-release = true/false`, page/limit forwarding, default query omission apart from pagination defaults used by the stream helper, documented 404 propagation, read-only retry behavior, Swagger audit coverage, README/CHANGELOG updates, and public API snapshot refresh.
- Improve live release asset-list confidence without making default integration tests brittle: when `GITEA_RELEASE_ASSET_ID` is set, the release asset-list live probe should assert that the configured asset id appears in `client.releaseAssets(owner, repo, releaseId)`; when it is absent, the probe may continue to accept an empty list as a valid read-only endpoint check.
- After release-list filtering is modeled, a low-risk adjacent read-only candidate is `GET /repos/{owner}/{repo}/releases/latest` (`repoGetLatestRelease`) because it reuses `Release`, has no new query/body/download semantics, and can extend the release metadata audit/live-probe surface without touching release writes or multipart upload contracts.
