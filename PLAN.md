...
- `PullRequestsApi` is mixed into `GiteaClient` with unambiguous `client.pullRequests(owner, repo, params)`, `client.pinnedPullRequests(owner, repo)`, `client.pullRequestByBaseHead(owner, repo, base, head)`, and `client.pullRequest(owner, repo, index)` facade methods.
- Pull request list responses decode through the existing paginated JSON mapper as `#/responses/PullRequestList`; pinned pull-request lists decode as non-paginated chunks from `#/responses/PullRequestList`; single pull request and base/head lookup responses decode as `#/responses/PullRequest`.
...
- `GiteaClientSpec` also covers multi-page pull request streaming, pinned pull-request listing, base/head pull-request lookup, and single-pull-request lookup through a `BackendStub[Task]`.
...
- likely continue with the next pull-request read slice from `plugin-redoc-2.yaml`, such as `GET /repos/{owner}/{repo}/pulls/{index}/commits` (`repoGetPullRequestCommits`) or `GET /repos/{owner}/{repo}/pulls/{index}/files` (`repoGetPullRequestFiles`), or move to the next small repository-management endpoint,
