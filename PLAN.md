...
- likely continue with another small issue-management slice from `plugin-redoc-2.yaml`, such as listing pinned issues through `GET /repos/{owner}/{repo}/issues/pinned` (`issueListPinned`) and checking whether new pins are allowed through `GET /repos/{owner}/{repo}/new_pin_allowed` (`repoNewIssuePinsAllowed`),
