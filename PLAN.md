...
- Highest-value next continuation: run the opt-in `client.releaseByTag(owner, repo, tag)` probe against a real Gitea repository only when a known slash-containing release tag such as `release/candidate` is available. Record the observed accept/reject behavior narrowly as `repoGetReleaseByTag` evidence in README, CHANGELOG, and this plan, including the exact configured tag value and whether the endpoint accepted the encoded slash path. If no such live repository state is available, keep slash-containing release-tag routing marked as pending observation rather than claiming it was verified.
- If the release-by-tag probe is used only with a normal tag such as `v1.0.0`, treat the result as generic release-by-tag live confidence, not slash-routing confidence. Keep README, CHANGELOG, and this plan explicit that normal-tag success does not prove slash-containing tag routing.
- If no slash-containing release tag is available for the next run, avoid another docs-only cycle that merely restates the same pending evidence. Preserve the pending marker, run the normal validation gate, and choose the next deliberate API slice only after explicitly deciding its contract boundary; release delete-by-tag, release writes, multipart asset upload/edit/delete, and asset binary download remain out of scope unless that deliberate design decision is made first.
