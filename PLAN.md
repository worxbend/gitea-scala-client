...
- Release-by-tag probe wiring remains available for the existing read-only `client.releaseByTag(owner, repo, tag)` facade. `LiveGiteaIntegrationSpec` has an opt-in probe gated on all five required variables being non-empty: `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, and `GITEA_RELEASE_TAG`. When enabled, the probe builds the live ZIO backend, passes the configured tag through unchanged to `client.releaseByTag(owner, repo, tag)`, and asserts that the returned `Release.tagName` contains exactly the configured `GITEA_RELEASE_TAG`; slash-containing values such as `release/candidate` are the intended confidence case.
- Release-by-tag evidence for this iteration is intentionally narrow. No enabled live repository run with a slash-containing `GITEA_RELEASE_TAG` is recorded here, so slash-containing release-tag routing for `repoGetReleaseByTag` remains pending live observation. README and CHANGELOG now distinguish a normal tag such as `v1.0.0` as generic release-by-tag confidence only, keep slash-routing evidence pending until a slash-containing tag is actually observed, and include `GITEA_RELEASE_TAG` in credential-stripped integration snippets.
- Focused documentation/continuation validation for this release-by-tag evidence slice records `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; the credential-stripped integration run proves hermetic skipping only and should not be treated as real release-tag routing evidence.
- Fresh senior review of the release-by-tag evidence/documentation slice found no functional blocker. The diff only changes README, CHANGELOG, PLAN, AGENT_LOG, and ALTERNATIVES telemetry; no production source or integration-test code changed in this iteration. The existing `LiveGiteaIntegrationSpec` probe remains correctly gated and read-only, and focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, JSONL validation for `ALTERNATIVES.jsonl`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials. Remaining risk is only evidence quality: slash-containing release-tag routing is still unobserved against real Gitea.
- Iteration 5 release-tag evidence gate is closed without a live observation. The current environment did not provide a complete real fixture with non-empty `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, and a slash-containing `GITEA_RELEASE_TAG`, so the opt-in `client.releaseByTag(owner, repo, tag)` probe was not run. No production code changed and no README/CHANGELOG live-verification claim was added; slash-containing release-tag routing for `repoGetReleaseByTag` remains explicitly pending live observation until an enabled run records the exact configured tag and accept/reject behavior.
- Forty-fourth typed repository collaborator read slice is complete for the three read-only Swagger `GET` operations under `/repos/{owner}/{repo}/collaborators`: `repoListCollaborators`, `repoCheckCollaborator`, and `repoGetRepoPermissions`. The public facade now exposes `ReposApi.collaborators(owner, repo)` as a paginated `ZStream` of `User`, `ReposApi.isCollaborator(owner, repo, collaborator)` as a `Boolean` membership check using the endpoint-specific `204`/`404` contract, and `ReposApi.collaboratorPermission(owner, repo, collaborator)` as `RepoCollaboratorPermission`.
- Collaborator request construction and decoding are schema-traceable and deliberately read-only. Owner, repository, and collaborator username values are encoded as path segments, collaborator list pagination starts at page 1 through the shared helper with the configured page size, all three requests are read-only retry eligible, the permission response is modeled as the minimal Swagger-shaped `RepoCollaboratorPermission` (`permission`, `role_name`, and `user`), and collaborator add/delete/write methods remain out of scope.
- Collaborator contract coverage is complete in the stub-backed tests and Swagger audit. `GiteaRequestsSpec` covers method/path/query/header/body contracts, success decoding, documented error mapping, and no JSON `Content-Type` on bodyless GETs; `GiteaClientSpec` covers facade pagination, slash/space path encoding, documented error propagation, and read-only retry behavior; `GiteaEndpointAuditSpec` covers operation IDs, methods, paths, required path parameters, optional pagination query names, success response refs, documented non-2xx labels, request-body absence, and retryability; `CoreModelsSpec` covers the new permission response codec and schema-field checklist entry.
- Public API snapshots are refreshed for the intentional collaborator ABI additions in `api-snapshot/client.txt` and `api-snapshot/core.txt`, including `RepoCollaboratorPermission`, the three `GiteaEndpoints` values, the three `GiteaRequests` builders, and the three `ReposApi` facade methods.
- Validation for the collaborator snapshot and plan refresh passed with `git diff --check`, `./mill --no-server compatibility.writeSnapshot`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials.
- Fresh senior review of the repository collaborator read slice found no functional blocker. The local Swagger confirms `repoListCollaborators`, `repoCheckCollaborator`, `repoGetRepoPermissions`, and `RepoCollaboratorPermission` fields; the implementation keeps collaborator writes absent, keeps documented non-2xx expectations test-private, and uses the existing endpoint-specific `204`/`404` boolean mapper only for `repoCheckCollaborator`. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored. Residual risk is semantic rather than a blocker: `repoCheckCollaborator` necessarily collapses the endpoint's documented `404` into `false`, so callers needing to distinguish missing repositories from absent collaborators should use a lookup/list/permission endpoint and handle errors explicitly.
- Repository team model gate is complete for `GET /repos/{owner}/{repo}/teams` (`repoListTeams`, `#/responses/TeamList`) and `GET /repos/{owner}/{repo}/teams/{team}` (`repoCheckTeam`, `#/responses/Team`). The local Swagger `Team` response is bounded to scalar fields, the existing `Organization` model, the existing `TeamPermission` enum, `units`, and `units_map`, so it can be modeled in core without pulling broad admin/write request payloads into the public API. `Team` now preserves `can_create_org_repo`, `includes_all_repositories`, and `units_map` wire names, keeps fields optional, and `CoreModelsSpec` covers single-team decode/round-trip, `TeamList` array decoding, unknown permission rejection, and a test-side schema-field checklist for all local `Team` fields.
- Repository team request construction is complete for the two read-only visibility endpoints: `GET /repos/{owner}/{repo}/teams` (`repoListTeams`) and `GET /repos/{owner}/{repo}/teams/{team}` (`repoCheckTeam`). `GiteaEndpoints` now records operation IDs, paths, methods, Swagger-declared required path parameters, success response refs, and read-only retry eligibility; `GiteaRequests.repoTeams` safely encodes owner/repo, sends `page` and configured `limit` query parameters, and decodes `TeamList` as `Page[Team]`; `GiteaRequests.repoTeam` safely encodes owner/repo/team with slash/space team names as one path segment, sends no query parameters, and decodes a single `Team`.
- Repository team request/audit coverage is complete in the HTTP layer. `GiteaRequestsSpec` covers methods, paths, list pagination query parameters, lookup no-query behavior, shared JSON accept/auth/OTP/user-agent headers, no body or JSON `Content-Type`, success decoding, documented `404` and `405` error mapping, and read-only retryability. `GiteaEndpointAuditSpec` compares both endpoints against `plugin-redoc-2.yaml` with private documented non-2xx expectations and additionally asserts the required list request `page`/`limit` query pairs, while preserving the local Swagger fact that `repoListTeams` does not declare those optional query parameters.
- Focused validation for the repository team request slice passed with `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec`. `GiteaResponseMapper` required no production changes because the existing JSON object, paginated JSON array, 404, and 405 decoding paths cover this slice.
- Repository team facade wiring is complete for the read-only visibility surface. `ReposApi.teams(owner, repo)` streams paginated `Team` values from `repoListTeams` through the shared page-1 pagination helper with the configured page size, and `ReposApi.team(owner, repo, team)` fetches a single `Team` through `repoCheckTeam`; `SttpGiteaClient` sends both via `GiteaRequestExecutor` and does not expose team add/delete/write operations.
- Repository team facade coverage is complete in `GiteaClientSpec`: multi-page team streaming proves page-1 start semantics and configured `limit`, single-team lookup proves slash/space team names are encoded as one path segment, documented `404` and `405` errors propagate through the facade, and read-only retry behavior is covered for the single-team lookup. Focused validation passed with `./mill --no-server client.test.testOnly io.worxbend.gitea4s.GiteaClientSpec`.
- Documentation and public API snapshot alignment for the read-only repository team visibility surface is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/`. README and CHANGELOG document `client.teams(owner, repo)` and `client.team(owner, repo, team)`, state that team add/delete/write operations are not implemented, and keep the team endpoints described as read-only retryable visibility methods. Public API snapshots now capture the intentional `Team`, `GiteaEndpoints`, `GiteaRequests`, and `ReposApi` ABI additions. Validation passed with `git diff --check`, `./mill --no-server compatibility.writeSnapshot`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials.
- Fresh senior review of the repository team visibility slice found no functional blocker. The local Swagger confirms `repoListTeams`, `repoCheckTeam`, `TeamList`, `Team`, `TeamPermission`, and `units_map`; the implementation keeps team add/delete/write operations absent, keeps documented non-2xx expectations test-private, and uses the existing JSON/page/error mapper paths correctly. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored. Residual risk is live-contract confidence: `repoListTeams` does not declare `page` or `limit` in the local Swagger, but the client pragmatically streams it with those query parameters like other list endpoints, so an enabled live probe should verify real Gitea accepts that behavior before team workflows are broadened.
- Forty-fifth typed repository tag lookup slice is complete for `GET /repos/{owner}/{repo}/tags/{tag}` (`repoGetTag`) using the existing `Tag` core model. The request layer records endpoint metadata in `GiteaEndpoints`, builds a pure bodyless JSON-accepting GET in `GiteaRequests`, safely encodes owner, repo, and tag as path segments, sends shared auth/OTP/user-agent headers, sends no query parameters, no request body, and no JSON `Content-Type`, decodes a single `Tag`, propagates documented not-found errors through the existing mapper, and remains read-only retry eligible.
- Repository tag lookup facade wiring is complete with `ReposApi.tag(owner, repo, tag): IO[GiteaError, Tag]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. The existing `ReposApi.tags(owner, repo)` paginated stream remains unchanged, and no tag write/delete APIs or release-tag behavior were introduced.
- Repository tag lookup coverage is complete in the stub-backed request/facade tests and Swagger audit. `GiteaRequestsSpec` covers method/path/header/body contract, punctuation-heavy tags such as `v1.0.0`, slash-containing tags such as `release/candidate` encoded as one path segment, success decoding, documented not-found propagation, and read-only retry eligibility. `GiteaClientSpec` covers successful facade lookup, slash-containing tag path encoding through the backend stub, not-found propagation, and read-only retry behavior. `GiteaEndpointAuditSpec` compares `repoGetTag` against `plugin-redoc-2.yaml` for operation ID, method, path, required path parameters, success response ref, documented non-2xx labels, absence of query parameters, absence of request body, and read-only retryability.
- Documentation and validation alignment for the repository tag lookup slice is complete in `README.md`, `CHANGELOG.md`, this plan, and public API snapshots for the intentional `ReposApi.tag`, `GiteaEndpoints.repoGetTag`, and `GiteaRequests.repoTag` ABI additions. Fresh senior review found no functional blocker: the local Swagger confirms `repoGetTag` is a path-only read endpoint returning `#/responses/Tag` with documented `404`, request/facade tests cover punctuation-heavy and slash-containing tag values as one encoded path segment, and audit-only documented non-2xx expectations remain private to `GiteaEndpointAuditSpec`. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored. Slash-containing repository tag routing remains unit-tested only and still needs enabled real Gitea observation before being claimed as live evidence.
- Iteration 8 path selection is complete. The current environment does not provide a complete live fixture: `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, `GITEA_COLLABORATOR`, `GITEA_TEAM`, `GITEA_REPO_TAG`, and `GITEA_RELEASE_TAG` are all absent, so no collaborator/team/tag/release-tag live probe was added and no enabled-run evidence was produced. The iteration should proceed with the bounded read-only tag-protection metadata fallback rather than another documentation-only live evidence cycle.
- The local Swagger fallback contract is confirmed in `plugin-redoc-2.yaml`. `repoListTagProtection` is `GET /repos/{owner}/{repo}/tag_protections`, has required path parameters `owner` and `repo`, declares no query parameters and no request body, and has a `200` success response ref `#/responses/TagProtectionList` with no documented non-2xx responses. `repoGetTagProtection` is `GET /repos/{owner}/{repo}/tag_protections/{id}`, has required path parameters `owner`, `repo`, and integer `id`, declares no query parameters and no request body, has a `200` success response ref `#/responses/TagProtection`, and documents `404` as `#/responses/notFound`. `TagProtection` is narrowly bounded to `created_at`, `id` (`int64`), `name_pattern`, `updated_at`, `whitelist_teams`, and `whitelist_usernames`; `TagProtectionList` is an array of `TagProtection`. Tag-protection create/edit/delete write operations remain explicitly out of scope for the fallback.
- Forty-sixth typed repository tag-protection metadata slice is complete for `GET /repos/{owner}/{repo}/tag_protections` (`repoListTagProtection`) and `GET /repos/{owner}/{repo}/tag_protections/{id}` (`repoGetTagProtection`) with the Swagger-shaped `TagProtection` model. The model preserves `created_at`, `id`, `name_pattern`, `updated_at`, `whitelist_teams`, and `whitelist_usernames`, uses `Long` for ids, keeps version-sensitive fields optional, and has zio-json codec coverage plus a test-side schema-field checklist for every local Swagger `TagProtection` field.
- Repository tag-protection endpoint metadata and request construction are complete. `GiteaEndpoints` records operation IDs, paths, methods, required path parameters, success response refs, and read-only retry eligibility; `GiteaRequests.repoTagProtections` and `GiteaRequests.repoTagProtection` safely encode owner/repo/id path segments, send shared auth, OTP, user-agent, and `Accept: application/json` headers, send no request body and no JSON `Content-Type`, decode `TagProtectionList` as a non-paginated `Chunk[TagProtection]`, decode detail as `TagProtection`, and propagate the documented detail `404` through the existing mapper. The list endpoint remains non-paginated because the local Swagger declares no `page` or `limit` query parameters.
- Repository tag-protection facade wiring is complete with `ReposApi.tagProtections(owner, repo): IO[GiteaError, Chunk[TagProtection]]` and `ReposApi.tagProtection(owner, repo, id): IO[GiteaError, TagProtection]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. Tag-protection create, edit, and delete write operations remain deliberately absent.
- Repository tag-protection Swagger audit coverage is complete in `GiteaEndpointAuditSpec`, including method, path, operation ID, required path parameters, exact absence of query parameters, absence of request bodies, success response refs, read-only retryability, no documented non-2xx responses for the list endpoint, and private audit-only `404` response labels for the detail endpoint.
- Documentation and public API snapshot alignment for the read-only repository tag-protection metadata surface is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/`. README and CHANGELOG document `client.tagProtections(owner, repo)` and `client.tagProtection(owner, repo, id)`, state that tag-protection create/edit/delete write operations are not implemented, and describe the list endpoint as non-paginated because the local Swagger declares no query parameters. Public API snapshots now capture the intentional `TagProtection`, `GiteaEndpoints`, `GiteaRequests`, and `ReposApi` ABI additions. Validation passed with `./mill --no-server compatibility.writeSnapshot`, `git diff --check`, `./mill --no-server compatibility.check`, and `./mill --no-server core.test client.test`.
- Fresh senior review of the repository tag-protection metadata slice found no functional blocker. The local Swagger confirms `repoListTagProtection`, `repoGetTagProtection`, `TagProtectionList`, and `TagProtection`; the implementation keeps tag-protection writes absent, keeps documented non-2xx expectations test-private, and correctly treats the list endpoint as non-paginated because no query parameters are declared. Focused validation passed with `git diff --check`, JSONL validation for `ALTERNATIVES.jsonl`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored. Residual follow-ups are evidence and scope discipline: add tag-protection live confidence only with a real protected-tag fixture, and keep create/edit/delete out of scope until a deliberate write slice handles request payloads and documented 403/404/422/423 failures.
- Branch-protection model sizing is complete and acceptable as passive read-only metadata. The local Swagger `BranchProtection` response is broad but bounded to metadata fields, so the core model keeps version-sensitive fields optional, uses `Instant` for timestamps, preserves wire names with annotations, and covers approval whitelists, admin/official/outdated/rejected review blockers, deprecated `branch_name`, `created_at`, `updated_at`, `dismiss_stale_approvals`, `enable_*` booleans, force-push/merge/push allowlists, `ignore_stale_approvals`, `priority`, protected/unprotected file-pattern fields, `required_approvals`, `rule_name`, and `status_check_contexts`. `CoreModelsSpec` covers decode, round-trip, optional/unknown-field behavior, non-paginated list decoding, and a test-side schema-field checklist for every local Swagger `BranchProtection` field.
- Forty-seventh typed repository branch-protection metadata slice is complete for `GET /repos/{owner}/{repo}/branch_protections` (`repoListBranchProtection`) and `GET /repos/{owner}/{repo}/branch_protections/{name}` (`repoGetBranchProtection`) with the Swagger-shaped `BranchProtection` model. `GiteaEndpoints` records operation IDs, paths, methods, required path parameters, success response refs, and read-only retry eligibility; `GiteaRequests.repoBranchProtections` and `GiteaRequests.repoBranchProtection` safely encode owner/repo/name path segments, send shared auth, OTP, user-agent, and `Accept: application/json` headers, send no request body and no JSON `Content-Type`, decode `BranchProtectionList` as a non-paginated `Chunk[BranchProtection]`, decode detail as `BranchProtection`, and propagate documented detail errors through the existing mapper. The list endpoint remains non-paginated because the local Swagger declares no `page`, `limit`, or other query parameters.
- Repository branch-protection facade wiring is complete with `ReposApi.branchProtections(owner, repo): IO[GiteaError, Chunk[BranchProtection]]` and `ReposApi.branchProtection(owner, repo, name): IO[GiteaError, BranchProtection]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. Request and facade tests cover list success, detail success, slash-containing branch/rule names such as `release/2026` encoded as one path segment, documented not-found/error propagation, non-paginated list behavior, and read-only retry behavior. Branch-protection create, edit, delete, and priority write operations remain deliberately absent.
- Repository branch-protection Swagger audit coverage is complete in `GiteaEndpointAuditSpec`, including method, path, operation ID, required path parameters, exact absence of query parameters, absence of request bodies, success response refs, read-only retryability, and private audit-only documented non-2xx response labels for the detail endpoint.
- Documentation and validation alignment for the read-only repository branch-protection metadata surface is complete in `README.md`, `CHANGELOG.md`, and this plan. README and CHANGELOG document `client.branchProtections(owner, repo)` and `client.branchProtection(owner, repo, name)`, state that branch-protection lists are non-paginated because the local Swagger declares no query parameters, and state that branch-protection create/edit/delete/priority write operations are not implemented. Final validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; the credential-stripped integration run reported all twelve live probes ignored and is not live branch-protection evidence.
- Fresh senior review of the repository branch-protection metadata slice found no functional blocker. The local Swagger confirms `repoListBranchProtection`, `repoGetBranchProtection`, `BranchProtectionList`, and every `BranchProtection` response field; the implementation keeps branch-protection writes absent, keeps documented non-2xx expectations test-private, treats the list endpoint as non-paginated because no query parameters are declared, and preserves slash-containing `{name}` values as one encoded path segment in request/facade tests. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored. Residual risks are evidence and ABI size: branch-protection live behavior is unobserved without a configured rule fixture, and the wide optional-field case class is now public ABI, so future edits should avoid churn unless Swagger drift demands it.
- Repository language statistics Swagger contract confirmation is complete for `GET /repos/{owner}/{repo}/languages` (`repoGetLanguages`). The local `plugin-redoc-2.yaml` operation has required path parameters `owner` and `repo`, declares no query parameters and no request body, returns `#/responses/LanguageStatistics` on success, and documents `404` as `#/responses/notFound`.
- Forty-eighth typed repository language-statistics slice is complete with `LanguageStatistics` as a public core model for the map-shaped Gitea response from language name to byte count. The zio-json codec preserves the plain JSON object wire shape such as `{"Scala":1234,"Java":55}` rather than adding a wrapper field, uses `Long` byte counts, handles empty maps, and rejects non-numeric language values.
- Repository language request and facade wiring is complete for `GiteaRequests.repoLanguages(config, owner, repo)` and `ReposApi.languages(owner, repo): IO[GiteaError, LanguageStatistics]`. The request safely encodes owner and repo as path segments, sends shared auth, OTP, user-agent, and `Accept: application/json` headers, sends no query parameters, sends no request body, sends no JSON `Content-Type`, decodes successful map-shaped responses, propagates documented `404` failures through the existing mapper, and is read-only retry eligible through `GiteaEndpoints.repoGetLanguages`.
- Repository language contract coverage is complete in the core, request, audit, and facade test layers. `CoreModelsSpec` covers decode, encode/round-trip, empty map handling, non-numeric rejection, and a test-side schema-field assertion for the dynamic map response. `GiteaRequestsSpec`, `GiteaEndpointAuditSpec`, and `GiteaClientSpec` cover method/path/header/body contract, exact absence of query/body surface, `#/responses/LanguageStatistics`, private documented non-2xx expectations, successful multi-language and empty-map decoding, documented not-found propagation, and read-only retry behavior. Repository language write behavior remains absent.
- Documentation and public API snapshot alignment for the repository language-statistics slice is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/`. README and CHANGELOG document `client.languages(owner, repo)` as a read-only path-only repository metadata method returning language byte counts and state that repository language write behavior is not implemented. Public API snapshots now capture the intentional `LanguageStatistics`, `GiteaEndpoints.repoGetLanguages`, `GiteaRequests.repoLanguages`, and `ReposApi.languages` ABI additions. Validation passed with `git diff --check`, `./mill --no-server compatibility.writeSnapshot`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials.
- Fresh senior review of the repository language-statistics slice found no functional blocker. The local Swagger confirms `repoGetLanguages` and `LanguageStatistics` as a path-only `additionalProperties` `int64` map response with documented `404`; the implementation keeps the wire shape unwrapped, keeps documented non-2xx expectations test-private, uses the existing read-only retry and error-mapping paths, and adds focused request/facade/audit/model coverage. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, and `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`. Residual follow-ups are small: add explicit fractional and overflow-number decode assertions if another dynamic map-shaped response is introduced, and consider extracting a tiny `additionalProperties` codec helper only after a second map-shaped response appears.
- Forty-ninth typed repository assignee metadata slice is complete for `GET /repos/{owner}/{repo}/assignees` (`repoGetAssignees`) using the existing `User` model. `GiteaEndpoints` records the operation ID, path, method, required `owner`/`repo` path parameters, `#/responses/UserList` success response, and read-only retry eligibility; `GiteaRequests.repoAssignees` safely encodes owner/repo path segments, sends shared auth, OTP, user-agent, and `Accept: application/json` headers, sends no query parameters, no request body, and no JSON `Content-Type`, decodes the `UserList` response as a non-paginated `Chunk[User]`, and propagates documented `404` failures through the existing mapper.
- Repository assignee facade wiring is complete with `ReposApi.assignees(owner, repo): IO[GiteaError, Chunk[User]]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. The facade deliberately returns a chunk rather than a stream because the local Swagger endpoint declares no `page`, `limit`, or other query parameters. Assignee assignment and removal write operations remain deliberately absent.
- Repository assignee request/facade/audit coverage is complete in the stub-backed tests and Swagger audit. `GiteaRequestsSpec` and `GiteaClientSpec` cover GET method/path/header/body contract, safe owner/repo path encoding, exact absence of query parameters, successful JSON user-array decoding, documented `404` propagation, no JSON `Content-Type`, and read-only retry behavior. `GiteaEndpointAuditSpec` compares `repoGetAssignees` against `plugin-redoc-2.yaml` for operation ID, method, path, required path parameters, exact no-query/no-body contract, `#/responses/UserList`, private documented `404` response labels, and retryability.
- Documentation and public API snapshot alignment for the repository assignee metadata surface is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/`. README and CHANGELOG document `client.assignees(owner, repo)` as a read-only path-only, non-paginated metadata method returning `Chunk[User]`, and state that assignee assignment/removal write behavior is not implemented. Public API snapshots capture the intentional `ReposApi.assignees`, `GiteaEndpoints.repoGetAssignees`, and `GiteaRequests.repoAssignees` ABI additions. Validation passed with `git diff --check`, `./mill --no-server compatibility.writeSnapshot`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials.
- Fiftieth typed repository social metadata slice is complete for `GET /repos/{owner}/{repo}/reviewers` (`repoGetReviewers`), `GET /repos/{owner}/{repo}/stargazers` (`repoListStargazers`), and `GET /repos/{owner}/{repo}/subscribers` (`repoListSubscribers`) using the existing `User` model. `GiteaEndpoints` records operation IDs, paths, methods, required `owner`/`repo` path parameters, `#/responses/UserList` success responses, and read-only retry eligibility for all three endpoints.
- Repository social metadata request construction is complete. `GiteaRequests.repoReviewers` safely encodes owner/repo path segments, sends shared auth, OTP, user-agent, and `Accept: application/json` headers, sends no query parameters, no request body, and no JSON `Content-Type`, and decodes `UserList` as a non-paginated `Chunk[User]`. `GiteaRequests.repoStargazers` and `GiteaRequests.repoSubscribers` safely encode owner/repo, send `page` and configured `limit` query parameters, send no request body and no JSON `Content-Type`, and decode paginated `Page[User]` responses.
- Repository social metadata facade wiring is complete with `ReposApi.reviewers(owner, repo): IO[GiteaError, Chunk[User]]`, `ReposApi.stargazers(owner, repo): ZStream[Any, GiteaError, User]`, and `ReposApi.watchers(owner, repo): ZStream[Any, GiteaError, User]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. The public `watchers` name maps to the Swagger `/subscribers` endpoint while request and endpoint names remain traceable to `repoListSubscribers`. Star, watch, and reviewer write behavior remains deliberately absent.
- Repository social metadata request/facade/audit coverage is complete in the stub-backed tests and Swagger audit. `GiteaRequestsSpec` covers method/path/header/body contracts, safe owner/repo path encoding with spaces and slashes, reviewer no-query behavior, stargazer/subscriber pagination query parameters, successful user-list decoding, documented reviewer `404`, stargazer `403`/`404`, and subscriber `404` propagation, no JSON `Content-Type`, and read-only retryability. `GiteaClientSpec` covers reviewer success, stargazer and watcher multi-page streaming, configured page-size forwarding, path encoding, documented not-found/forbidden propagation, and read-only retry behavior. `GiteaEndpointAuditSpec` compares operation IDs, methods, paths, required path parameters, exact query/body surface, `#/responses/UserList`, private documented non-2xx labels, and retryability against `plugin-redoc-2.yaml`.
- Documentation, public API snapshot, and validation alignment for the repository social metadata surface is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/`. README and CHANGELOG document `client.reviewers(owner, repo)` as `Chunk[User]`, `client.stargazers(owner, repo)` as a paginated stargazer stream, and `client.watchers(owner, repo)` as a paginated watcher stream backed by `/subscribers`, and state that star/watch/reviewer write behavior is not implemented. Validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES -u GITEA_BRANCH_PROTECTION_NAME -u GITEA_TAG_PROTECTION_ID ./mill --no-server it.test`; all twelve live probes were ignored without credentials, so no live evidence is claimed for reviewers, stargazers, or watchers.
- Fresh senior review of the repository social metadata slice found no functional blocker. The local Swagger confirms `repoGetReviewers`, `repoListStargazers`, and `repoListSubscribers`; the implementation keeps star/watch/reviewer writes absent, keeps documented non-2xx expectations test-private, exposes `/subscribers` as the public watcher stream with traceable low-level names, and correctly treats reviewers as non-paginated while stargazers/subscribers use page/limit streams. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, JSONL validation for `ALTERNATIVES.jsonl`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES -u GITEA_BRANCH_PROTECTION_NAME -u GITEA_TAG_PROTECTION_ID ./mill --no-server it.test`; all twelve live probes were ignored. Residual risks are evidence-level: no enabled live run has observed reviewers/stargazers/watchers behavior, and the repeated inline request-recording backend pattern in stream facade tests should be extracted before another sequential-query streaming test is added.
- Fifty-first typed repository GPG signing-key read slice is complete for `GET /repos/{owner}/{repo}/signing-key.gpg` (`repoSigningKey`). The request layer records operation ID, method, exact path, required `owner`/`repo` path parameters, `text/plain` success response semantics, and read-only retry eligibility; `GiteaRequests.repoSigningKey` safely encodes owner and repo as path segments, sends shared auth/OTP/user-agent headers, sends `Accept: text/plain`, sends no query parameters, no request body, and no JSON `Content-Type`, decodes successful text bodies as `String`, and preserves existing non-2xx raw-body/error behavior.
- Repository GPG signing-key facade wiring is complete with `ReposApi.gpgSigningKey(owner, repo): IO[GiteaError, String]` implemented in `SttpGiteaClient` through `GiteaRequestExecutor`. The facade is deliberately text-returning and read-only; SSH signing-key, default `/signing-key.*` endpoints, repository key CRUD, writes, multipart, hooks, and public raw request execution surfaces remain absent.
- Repository GPG signing-key request/facade/audit coverage is complete in the stub-backed tests and Swagger audit. `GiteaRequestsSpec` verifies GET method, `/api/v1/repos/{owner}/{repo}/signing-key.gpg` path construction, safe owner/repo segment encoding, exact no-query/no-body behavior, no JSON `Content-Type`, `Accept: text/plain`, shared auth/OTP/user-agent headers, successful raw text decoding, existing non-2xx mapping, and read-only retry metadata. `GiteaClientSpec` covers successful facade text decoding, owner/repo path encoding through the backend stub, representative non-2xx propagation, and read-only retry behavior. `GiteaEndpointAuditSpec` compares operation ID, method, exact path, required path parameters, absence of query parameters and request body, `text/plain` production, `200` string schema, no documented non-2xx responses, and retryability against `plugin-redoc-2.yaml`.
- Documentation, public API snapshot, and validation alignment for the repository GPG signing-key slice is complete in `README.md`, `CHANGELOG.md`, this plan, and `api-snapshot/client.txt`. README and CHANGELOG document `client.gpgSigningKey(owner, repo)` as a small read-only `text/plain` endpoint returning `String`, and state that SSH signing-key, default signing-key, key management, and write endpoints are not implemented. Validation passed with `git diff --check`, `./mill --no-server compatibility.writeSnapshot`, `./mill --no-server core.test client.test compatibility.check`, focused `./mill --no-server client.test.testOnly io.worxbend.gitea4s.http.GiteaRequestsSpec io.worxbend.gitea4s.http.GiteaEndpointAuditSpec io.worxbend.gitea4s.GiteaClientSpec`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES -u GITEA_BRANCH_PROTECTION_NAME -u GITEA_TAG_PROTECTION_ID ./mill --no-server it.test`; all twelve live probes were ignored without credentials, so no live signing-key evidence is claimed.
- Fresh senior review of the repository GPG signing-key slice found no functional blocker. The local Swagger confirms `repoSigningKey` as `GET /repos/{owner}/{repo}/signing-key.gpg`, `produces: text/plain`, required `owner`/`repo` path parameters only, an inline `200` string schema, and no documented non-2xx responses. The implementation correctly keeps the successful text response out of JSON decoding, preserves the existing error mapper for non-2xx bodies, keeps audit-only response details private to `GiteaEndpointAuditSpec`, and does not broaden the public surface to SSH/default signing keys or key-management endpoints. Focused validation passed with `git diff --check`, JSONL validation for `ALTERNATIVES.jsonl`, `./mill --no-server core.test client.test compatibility.check`, focused request/audit/facade specs, and credential-stripped `it.test`; remaining risk is confidence-level only because no enabled live run observed a real repository signing key.

## Next Continuation

- Return to fixture-gated live confidence only when real fixtures are available. Require non-empty `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, and `GITEA_REPO`, plus at least one available confidence target among `GITEA_COLLABORATOR`, `GITEA_TEAM`, `GITEA_REPO_TAG`, or a slash-containing `GITEA_RELEASE_TAG`. Repository social metadata live confidence can use the same base repository fixture, but should assert configured non-empty identities such as `GITEA_REVIEWER`, `GITEA_STARGAZER`, and `GITEA_WATCHER` when those variables are introduced; empty-list success is weak evidence. Keep default `it.test` hermetic and do not treat credential-stripped ignored runs as live evidence.
- Consider opt-in branch-protection and tag-protection live probes only when a repository fixture has configured protection rules and adequate token permissions. Gate branch-protection with explicit variables such as `GITEA_BRANCH_PROTECTION_NAME` and tag-protection with explicit variables such as `GITEA_TAG_PROTECTION_ID`; assert both list membership and detail lookup for the configured rule, and do not treat empty-list success as strong confidence.
- If no live fixtures are available, avoid another evidence-only documentation loop and proceed with a small read-only Swagger fallback only after confirming the exact local contract. Good next candidates are bounded repository metadata endpoints that reuse existing models or simple response bodies and avoid write, hook, key, search, admin, action, multipart, and broad polymorphic download surfaces. `repoSigningKey` is complete; do not add `repoSigningKeySSH`, default `/signing-key.*` endpoints, or repository key management by adjacency unless a separate slice explicitly selects and audits that exact Swagger contract.
- Keep `BranchProtection` ABI stable unless local Swagger changes. Because the model has many optional constructor parameters, future changes should prefer additive schema-driven fields only, with schema-field checklist updates and compatibility snapshot refresh; do not refactor it into smaller public submodels unless there is a deliberate breaking-change decision.
- Document or test the `repoCheckCollaborator` ambiguity if user-facing docs grow: the public boolean method follows the endpoint-specific `204`/`404` membership contract, but `404` may be indistinguishable from some missing-resource cases at the HTTP layer.
- Keep repository GPG signing-key live confidence separate from request/facade correctness. Add an opt-in live probe only when a real repository fixture is known to expose a GPG signing key, and assert a narrow public-key property such as a non-empty armored block without recording live evidence from credential-stripped ignored runs.
- If another `text/plain` string endpoint is implemented, extract the new signing-key audit helper into a reusable test-private scalar/text audit path and add one endpoint-specific non-JSON failure-body assertion. Keep the current one-off helper local until a second text endpoint proves the abstraction is worth carrying.
- Keep slash-containing release-tag and repository-tag routing pending live observation. If repository tag live confidence is added, gate it on `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, and an explicit repository-tag variable such as `GITEA_REPO_TAG`; keep `GITEA_RELEASE_TAG` reserved for `client.releaseByTag`. Record the exact configured tag and accept/reject behavior in README, CHANGELOG, and this plan only after an enabled run succeeds or fails against real Gitea. Normal tags such as `v1.0.0` remain generic tag lookup confidence only.
- Before adding another streaming facade that needs query-sequence assertions, extract the ad hoc recording backend pattern from `GiteaClientSpec` into a small local test helper instead of copying another inline `Backend[Task]` implementation. The release, team, stargazer, and watcher tests now have enough similar shape to justify extraction on the next use.
- Keep release delete-by-tag, release create/edit/delete, release asset upload/edit/delete, and release asset binary download out of the next continuation unless a deliberate release-write, upload, or download design slice is selected. Multipart/form-data upload and shared `Attachment` versus `ReleaseAsset` naming must be decided explicitly before those endpoints are implemented.
...
- Keep README, CHANGELOG, public API snapshots, and this PLAN aligned with newly implemented API surface and validation results; run `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused request/audit/facade tests when code changes, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_COLLABORATOR -u GITEA_TEAM -u GITEA_REPO_TAG -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES -u GITEA_BRANCH_PROTECTION_NAME -u GITEA_TAG_PROTECTION_ID ./mill --no-server it.test` before the next review.
