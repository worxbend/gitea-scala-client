...
- If the live fixture gate is still incomplete, proceed with the next bounded read-only Swagger fallback: branch-protection metadata. Start with an explicit model-sizing gate because `BranchProtection` is much wider than `TagProtection`: confirm every local Swagger field, including `approvals_whitelist_teams`, `approvals_whitelist_username`, `block_admin_merge_override`, `block_on_official_review_requests`, `block_on_outdated_branch`, `block_on_rejected_reviews`, deprecated `branch_name`, `created_at`, `dismiss_stale_approvals`, `enable_*` booleans, force-push/merge/push allowlists, `priority`, file-pattern fields, `required_approvals`, `rule_name`, `status_check_contexts`, and `updated_at`. Implement only read APIs for `GET /repos/{owner}/{repo}/branch_protections` (`repoListBranchProtection`) and `GET /repos/{owner}/{repo}/branch_protections/{name}` (`repoGetBranchProtection`) if the model remains acceptable as an optional-field metadata response; keep branch-protection create/edit/delete/priority writes out of scope.
- Branch-protection request tests should prove the list endpoint is non-paginated because the local Swagger declares no query parameters, and the detail endpoint should encode slash-containing branch/rule names such as `release/2026` as one path segment before any broader branch-like routing convention is claimed. Add schema-field checklist coverage for the full `BranchProtection` response and keep documented non-2xx audit labels private to `GiteaEndpointAuditSpec`.
- Consider an opt-in tag-protection live probe only when a repository fixture has at least one configured tag-protection rule and adequate token permissions. Gate it with explicit variables such as `GITEA_TAG_PROTECTION_ID` and do not treat empty-list success as strong confidence unless a configured id is asserted in the list result.
