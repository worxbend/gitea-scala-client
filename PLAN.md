...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, Git tree, Git blob, Git refs, repository contents, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, Git tree `recursive`/`page`/`per_page` query toggles, repository contents `ref` query handling, Git blob and Git refs no-query/no-body checks, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit note, Git tree, Git blob, Git refs, repository contents metadata, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- add an opt-in live-confidence probe for `ReposApi.contents(owner, repo, filepath, ContentsParams)` using a slash-containing filepath such as `docs/readme.md`, gated on `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, `GITEA_CONTENTS_FILEPATH`, and optional `GITEA_CONTENTS_REF`, so the contents filepath routing convention is checked against real Gitea before more file-like endpoints rely on it,
- before implementing raw/media file reads, explicitly decide whether to introduce a byte-oriented `GiteaRequest`/response path or expose buffered text only for documented text-like downloads; do not accidentally decode arbitrary `application/octet-stream` responses through the existing JSON/string-only assumptions,
- implement the next small Swagger-backed raw repository file read slice from `plugin-redoc-2.yaml`: start with `GET /repos/{owner}/{repo}/raw/{filepath}` (`repoGetRawFile`) and, if the response boundary stays simple, `GET /repos/{owner}/{repo}/media/{filepath}` (`repoGetRawFileOrLFS`) with optional `ref` query handling, safe slash-containing `filepath` path encoding, `application/octet-stream` acceptance, deliberately chosen raw byte/string response semantics, and documented `404` propagation,
- add private endpoint audit expectations for `repoGetRawFile` and any included media/LFS endpoint, covering operation IDs, methods, paths, required path parameters, optional `ref` query parameter, `type: file` success response shape, request-body absence, retryability, and documented `404` response status/ref labels,
- explicitly test slash-containing raw/media `filepath` values as one encoded path parameter and keep the request tests clear about whether `{ref}/{filepath}` is supplied through the path parameter, the `ref` query parameter, or both,
- keep the GitHub-compatible `contents` API behavior anchored to the local Swagger response refs; defer broader directory/file polymorphism work to a deliberate `contents-ext` slice rather than widening `ReposApi.contents` implicitly,
