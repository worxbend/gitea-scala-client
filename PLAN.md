...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, Git tree, Git blob, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, Git tree `recursive`/`page`/`per_page` query toggles, Git blob no-query/no-body checks, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single repository commit lookup, commit note lookup, Git tree lookup, commit diff/patch raw text lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull, single-commit, commit-note, Git tree, and commit diff/patch GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit note, Git tree, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- implement the adjacent Swagger-backed Git refs read slice: `GET /repos/{owner}/{repo}/git/refs` (`repoListAllGitRefs`) and `GET /repos/{owner}/{repo}/git/refs/{ref}` (`repoListGitRefs`), both returning `#/responses/ReferenceList`,
- add minimal typed core model/codecs for the Swagger `Reference` and `GitObject` definitions required by `ReferenceList`; keep uncertain object fields optional and preserve Swagger JSON names,
- add endpoint metadata with required owner/repo path parameters for `repoListAllGitRefs`, required owner/repo/ref path parameters for `repoListGitRefs`, no query parameters, no request bodies, success response label `#/responses/ReferenceList`, read-only retry eligibility, and private audit expectations for documented `404` response status/ref labels,
- add pure request builders with safe owner/repo/ref path encoding, JSON accept headers, shared auth/OTP/user-agent headers, non-paginated `Chunk[Reference]` decoding, documented `404` failure propagation, and read-only retry semantics; include a request test for slash-containing refs such as `heads/main` so the chosen encoded path behavior is intentional,
- expose explicit repository facade methods such as `gitRefs(owner: String, repo: String): IO[GiteaError, Chunk[Reference]]` and `gitRefs(owner: String, repo: String, ref: String): IO[GiteaError, Chunk[Reference]]` only if the overload shape remains unambiguous on `GiteaClient`; otherwise choose clearer names before snapshot refresh,
- add focused core/request/facade tests for codec behavior, method/path/header construction, no-query construction, success decoding, documented 404 mapping, retry behavior through `BackendStub`, and representative `Reference` values whose `ref` and nested object fields match Swagger,
- extend `GiteaEndpointAuditSpec` for both Git refs endpoints without adding audit-only fields back to published endpoint metadata, refresh public API snapshots after intentional signatures compile, then run `./mill core.test`, `./mill client.test`, and `./mill compatibility.check`; run `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD ./mill __.test it.test examples.run` when time permits and keep live tests hermetic,
