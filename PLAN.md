...
- Client HTTP also has schema-traceable pull-request review-list request construction for `GET /repos/{owner}/{repo}/pulls/{index}/reviews` (`repoListPullReviews`).
...
- likely continue with the next pull-request review detail/comment/write slice from `plugin-redoc-2.yaml`, or move to the next small commit-status or repository-management endpoint,
