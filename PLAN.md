...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, Git tree, Git blob, Git refs, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, Git tree `recursive`/`page`/`per_page` query toggles, Git blob and Git refs no-query/no-body checks, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single repository commit lookup, commit note lookup, Git tree lookup, Git refs lookup including slash-containing ref filters, commit diff/patch raw text lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull, single-commit, commit-note, Git tree, Git refs, and commit diff/patch GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit note, Git tree, Git blob, Git refs, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- implement the adjacent Swagger-backed annotated Git tag read slice: `GET /repos/{owner}/{repo}/git/tags/{sha}` (`GetAnnotatedTag`) returning `#/responses/AnnotatedTag`; keep it distinct from the already implemented repository tag-list endpoint and document that the Swagger operation only supports annotated tag objects, not lightweight tags,
- add minimal typed core model/codecs for the Swagger `AnnotatedTag` and `AnnotatedTagObject` definitions required by the response; reuse existing supporting types where they already match Swagger, keep uncertain fields optional, and preserve the `object` JSON field name when the Scala field name differs,
- add endpoint metadata with required `owner`, `repo`, and `sha` path parameters, no query parameters, no request body, success response label `#/responses/AnnotatedTag`, read-only retry eligibility, and private audit expectations for documented `400` and `404` response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, JSON accept headers, shared auth/OTP/user-agent headers, no query parameters, no request body, `AnnotatedTag` decoding, documented `400`/`404` failure propagation, and read-only retry semantics,
- expose an explicit repository facade method such as `annotatedTag(owner: String, repo: String, sha: String): IO[GiteaError, AnnotatedTag]` unless existing naming suggests a clearer non-conflicting shape before snapshot refresh,
- add focused core/request/facade tests for codec behavior, method/path/header construction, no-query construction, success decoding, documented `400`/`404` mapping, retry behavior through `BackendStub`, and representative annotated-tag payloads whose `object`, `tagger`, and `verification` fields match Swagger,
- extend `GiteaEndpointAuditSpec` for `GetAnnotatedTag` without adding audit-only fields back to published endpoint metadata, refresh public API snapshots after intentional signatures compile, then run `./mill core.test`, `./mill client.test`, and `./mill compatibility.check`; run `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD ./mill __.test it.test examples.run` when time permits and keep live tests hermetic,
- add or schedule an opt-in live integration probe for `client.gitRefs(owner, repo, "heads/main")` once a suitable repository/ref can be provided through environment variables; keep it skipped by default and do not block the annotated-tag slice on unavailable live data,
- consider a lightweight model/schema checklist or helper for newly added Swagger definitions if more Git object response models are added; the endpoint audit catches metadata drift but does not prove every response-model field is represented,
- keep README, CHANGELOG, and this PLAN aligned with the newly implemented API surface and validation results.
