...
- Next low-risk continuation after latest-release: extend the release metadata live confidence suite for `client.latestRelease(owner, repo)`. Add an optional `GITEA_LATEST_RELEASE_TAG` assertion, or compare the returned release to `GITEA_RELEASE_TAG` only when the configured repository state makes that relationship explicit. Keep the probe read-only and hermetic when credentials or the latest-release-specific assertion variable are absent; do not infer latest-release correctness from arbitrary release IDs. Document the new variable in the README live-probe section and include it in credential-stripped validation snippets.
