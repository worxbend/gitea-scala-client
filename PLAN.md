...
The next continuation should add opt-in live confidence for the already implemented read-only release metadata APIs and close the remaining release audit wording gap without expanding the public surface:

- register `repoListReleases` and `repoGetRelease` in `GiteaEndpointAuditSpec`, including documented non-2xx labels, optional release-list query parameter names (`draft`, `pre-release`, `page`, `limit`), request body absence, retryability, and success response refs, so README claims about audited release endpoints are literally true;
- if full release list/detail audit registration is deferred, narrow README wording to say exactly which release endpoints are currently covered by Swagger audit (`repoGetReleaseByTag`, `repoListReleaseAttachments`, and `repoGetReleaseAttachment`);
- add hermetic live probes in `LiveGiteaIntegrationSpec` for `ReleasesApi.release(owner, repo, id)`, `ReleasesApi.releaseByTag(owner, repo, tag)`, `ReleasesApi.releaseAssets(owner, repo, releaseId)`, and `ReleasesApi.releaseAsset(owner, repo, releaseId, assetId)`;
- gate the probes on explicit non-empty variables such as `GITEA_RELEASE_ID`, `GITEA_RELEASE_TAG`, and `GITEA_RELEASE_ASSET_ID` in addition to `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, and `GITEA_REPO`, so credential-free `it.test` still makes no network calls;
- document the new live variables in README and keep the credential-stripped integration-test snippets aligned with the full live-variable set;
- keep the probes read-only and metadata-only; do not add release create/edit/delete, release asset upload/edit/delete, or release asset binary download APIs;
- preserve the existing request contracts for slash-containing tag values and release asset metadata while validating only against an explicitly configured real instance;
- do not add `repoDeleteReleaseByTag` simply because it shares `/releases/tags/{tag}`; release deletion is a separate write slice with 204/404/422 semantics and non-retryable behavior;
...
- keep README, CHANGELOG, public API snapshots, and this PLAN aligned with the newly implemented API surface and validation results; run `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused request/audit/facade tests, and credential-stripped `it.test` before the next review.
