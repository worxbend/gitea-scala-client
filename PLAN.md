...
- If no live fixtures are available, avoid another evidence-only documentation loop and proceed with a small read-only Swagger fallback. The best current candidate is `GET /repos/{owner}/{repo}/languages` (`repoGetLanguages`): model `LanguageStatistics` as a map-shaped response from language name to byte count, add endpoint metadata/request/facade/audit tests, keep it path-only with no query/body surface, propagate documented `404`, and avoid broad repository search/write/hook/key surfaces.
