...
- Client HTTP also has a lightweight spec-trace audit for pull-request review lifecycle endpoint metadata against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, response labels, request-body presence, retryability, and no-body lifecycle POST content-type behavior.
...
- `GiteaResponseMapper` decodes successful JSON responses, non-paginated JSON arrays, paginated issue/repository/branch/tag/release/pull-request/review/reaction/commit-status/notification lists, object-shaped user-search and topic-name pages, repository pin-capacity responses, 204/unit responses, pull-request merge-status 204/404 booleans, Gitea error payloads, raw failure bodies, pagination headers, and rate-limit reset headers.
...
- `GiteaClientSpec` also covers pull-request merge, scheduled auto-merge cancellation, update-by-merge/update-by-rebase, representative forbidden/not-found/conflict propagation, and non-retryable write semantics through a `BackendStub[Task]`.
...
- Documentation update for the commit-status audit, combined-status pagination controls, and pull-request merge/update facade is complete in `README.md`, `CHANGELOG.md`, and this plan. Validation for this documentation-only update was `git diff --check`; no Mill test command was required by this assigned task.
- Fresh review of the twenty-eighth slice found no request-construction blocker. Residual hardening work remains around merge/update metadata audit coverage, documented failure-status coverage, and the current generic mapping of unmodeled non-5xx statuses such as `405` and `423` to `GiteaError.ServerError`.
...
- before adding another write surface, extend `GiteaEndpointAuditSpec` to cover `repoMergePullRequest`, `repoCancelScheduledAutoMerge`, and `repoUpdatePullRequest`, including operation ID, method, path, required path parameters, success response labels, request-body presence, retryability, `style` query enum values, and documented non-2xx response status/ref labels,
- harden error handling for documented pull-request merge/update failures: decide whether to add explicit ADT cases for `405 Method Not Allowed` and `423 Locked`/repo archived responses or document and test the existing fallback; avoid leaving resource-state failures classified only as `ServerError`,
- make the Swagger audit fail loudly when path/method/parameter lookup fails and broaden it so future endpoint groups can compare documented failure statuses without duplicating parser logic,
- then add the next Swagger-backed pull-request write slice that complements merge/update, likely pull-request creation/editing or another small lifecycle command group from `plugin-redoc-2.yaml`,
...
- keep write facade methods small and typed, with request-layer tests first and `BackendStub` facade tests for success, representative errors, and retry behavior, including newly modeled non-5xx statuses where applicable,
