...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, Git tree, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, Git tree `recursive`/`page`/`per_page` query toggles, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- first decide whether to add a default argument to `ReposApi.gitTree(..., params: GitTreeParams = GitTreeParams.default)` for consistency with `commit` and `commitNote`; if changed, update `SttpGiteaClient`, README examples, facade tests, and the public API snapshot before adding more repository Git surface,
- implement the adjacent Swagger-backed Git blob read slice, `GET /repos/{owner}/{repo}/git/blobs/{sha}` (`GetBlob`), returning the documented `GitBlobResponse` JSON response,
- add a minimal typed core model/codecs for `GitBlobResponse` from `plugin-redoc-2.yaml`: `content`, `encoding`, `lfs_oid`, `lfs_size`, `sha`, `size`, and `url`; preserve schema JSON names with annotations for `lfs_oid` and `lfs_size`,
- keep blob content as the documented response object rather than decoding the base64 payload into bytes at this layer; expose decoded bytes later only after API ergonomics and large-blob behavior are validated,
- add endpoint metadata with method `GET`, path `/repos/{owner}/{repo}/git/blobs/{sha}`, operation ID `GetBlob`, required owner/repo/sha path parameters, no query parameters, success response label `#/responses/GitBlobResponse`, no request body, read-only retry eligibility, and private audit expectations for documented `400` and `404` response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, JSON accept headers, shared auth/OTP/user-agent headers, `GitBlobResponse` response decoding, documented `400`/`404` failure propagation, and read-only retry semantics,
- expose a typed repository facade method with an explicit name such as `gitBlob(owner: String, repo: String, sha: String): IO[GiteaError, GitBlobResponse]` if that naming fits the surrounding repository Git API,
- add focused core/request/facade tests for codec behavior, method/path/header construction, no-query/default construction, success decoding, documented 400/404 mapping, and retry behavior through `BackendStub`,
- extend `GiteaEndpointAuditSpec` for `GetBlob` without adding audit-only fields back to published endpoint metadata, and keep no-query/no-body checks explicit for repository Git endpoints,
