...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single repository commit lookup, commit note lookup, commit diff/patch raw text lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull, single-commit, commit-note, and commit diff/patch GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit note, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- implement the adjacent Swagger-backed Git tree read slice, `GET /repos/{owner}/{repo}/git/trees/{sha}` (`GetTree`), returning the documented `GitTreeResponse` JSON response,
- add minimal typed core models/codecs for `GitTreeResponse` and nested `GitEntry` from `plugin-redoc-2.yaml`: `GitTreeResponse.page`, `sha`, `total_count`, `tree`, `truncated`, and `url`; `GitEntry.mode`, `path`, `sha`, `size`, `type`, and `url`; use optional fields and preserve schema JSON names,
- do not model Git tree results as the existing `Page[A]` stream yet; Swagger returns pagination state in the response body through `page`, `total_count`, and `truncated`, so first expose the exact response object and revisit streaming only after the contract is understood against live behavior,
- add a small `GitTreeParams` model for the documented optional `recursive`, `page`, and `per_page` query parameters without introducing shared abstraction unless duplication becomes meaningful; preserve the wire name `per_page` exactly in request and audit tests,
- add endpoint metadata with method, path, operation ID `GetTree`, required owner/repo/sha path parameters, optional query parameter names, success response label `#/responses/GitTreeResponse`, no request body, read-only retry eligibility, and private audit expectations for documented `400` and `404` response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, JSON accept headers, shared auth/OTP/user-agent headers, optional query encoding, `GitTreeResponse` response decoding, documented `400`/`404` failure propagation, and read-only retry semantics,
- expose a typed repository facade method with an explicit name such as `gitTree(owner: String, repo: String, sha: String, params: GitTreeParams): IO[GiteaError, GitTreeResponse]` if that naming fits the surrounding repository Git API,
- add focused core/request/facade tests for codec behavior, method/path/query/header construction, default query omission, explicit query encoding, success decoding, documented 400/404 mapping, and retry behavior through `BackendStub`,
- extend `GiteaEndpointAuditSpec` for `GetTree` without adding audit-only fields back to published endpoint metadata, keep optional query parameter audits explicit for repository Git endpoints, and make sure the uppercase Swagger operation ID `GetTree` is compared exactly,
- refresh public API snapshots after intentional signatures compile, then run `./mill core.test`, `./mill client.test`, and `./mill compatibility.check`; run `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD ./mill __.test it.test examples.run` when time permits and keep live tests hermetic,
