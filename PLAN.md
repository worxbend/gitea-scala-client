...
- Highest-value next continuation remains fixture-gated live confidence for the read-only repository collaborator and team visibility surfaces, but only when real fixtures are available. Require `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, plus explicit fixture variables such as `GITEA_COLLABORATOR` and `GITEA_TEAM`; keep default `it.test` hermetic and do not treat credential-stripped ignored runs as live collaborator or team evidence.
- If the team live probe is added, assert `client.teams(owner, repo)` can observe the configured team name when permissions allow and assert `client.team(owner, repo, team)` returns the same configured team. The list probe should specifically validate the current `page`/`limit` query behavior because local Swagger omits those query parameters for `repoListTeams`; if real Gitea rejects or ignores that shape, revisit whether the facade should be non-paginated or should omit undocumented query parameters. Record slash/space team-name routing confidence only after an enabled run observes that exact configured value against real Gitea.
- Before adding a live collaborator probe, require a real repository fixture with a known collaborator account and adequate token permissions. If added, gate it on explicit variables such as `GITEA_COLLABORATOR` and optionally assert `collaboratorPermission`; keep default `it.test` hermetic and do not treat credential-stripped ignored runs as live collaborator evidence.
...
- If no suitable live collaborator/team/tag fixtures are available, avoid another documentation-only cycle and choose another bounded read-only Swagger slice with a clear contract, existing or narrowly modelable response types, no write side effects, and straightforward audit coverage. A good fallback is the read-only tag-protection metadata surface, `GET /repos/{owner}/{repo}/tag_protections` (`repoListTagProtection`) and `GET /repos/{owner}/{repo}/tag_protections/{id}` (`repoGetTagProtection`), after first confirming the `TagProtection` model remains narrowly bounded to the local Swagger fields. Keep create/edit/delete tag-protection writes out of that slice.
...
- Keep README, CHANGELOG, public API snapshots, and this PLAN aligned with newly implemented API surface and validation results; run `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused request/audit/facade tests when code changes, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test` before the next review.
