...
- `CoreModelsSpec` also carries a test-side schema-field checklist for recent Swagger Git response models (`Reference`, `GitObject`, `AnnotatedTag`, `AnnotatedTagObject`, and `GitBlobResponse`) so missing local field expectations fail loudly without adding schema-field metadata to public production APIs.
...
- Fresh senior review of the live-confidence probe and schema-field checklist slice found no functional blocker. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, and `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD ./mill --no-server it.test`; the integration run reported all four live tests as ignored. Residual follow-ups are process-level: the schema checklist is still hand-maintained rather than parsed from Swagger, and the workspace contains an untracked `ALTERNATIVES.jsonl` orchestration artifact that should be removed or deliberately ignored before committing.
...
- implement the next small Swagger-backed repository contents metadata read slice from `plugin-redoc-2.yaml`: start with `GET /repos/{owner}/{repo}/contents` (`repoGetContentsList`) and, if the model boundary stays small, `GET /repos/{owner}/{repo}/contents/{filepath}` (`repoGetContents`) with `ContentsResponse`, optional `ref` query handling, safe slash-containing `filepath` path encoding, and response decoding for `ContentsListResponse`/`ContentsResponse`,
- before coding the contents slice, add the `ContentsResponse` Swagger field list to the test-side schema-field checklist and add `FileLinksResponse` as a nested checklist entry for `_links`; keep the expected field list visibly copied from `plugin-redoc-2.yaml` or refactor the test helper to parse Swagger definitions directly,
- add private endpoint audit expectations for `repoGetContentsList` and `repoGetContents`, covering operation IDs, methods, paths, required path parameters, optional `ref` query parameter, success response labels, request-body absence, retryability, and documented `404` response status/ref labels,
- explicitly test slash-containing `filepath` values such as `docs/readme.md` as one encoded path parameter and decide whether an opt-in live routing probe is warranted before generalizing the same path convention to more contents-like endpoints,
- continue adding private endpoint audit expectations for documented non-2xx response labels from the start of each endpoint slice without reintroducing audit-only fields to published endpoint metadata,
- clean up or intentionally ignore untracked orchestration artifacts such as `ALTERNATIVES.jsonl` before the next commit so release diffs stay source-focused,
- keep README, CHANGELOG, public API snapshots, and this PLAN aligned with the newly implemented API surface and validation results; run `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, and focused request/audit/facade tests before the next review.
