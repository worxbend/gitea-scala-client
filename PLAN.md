...
- `CoreModelsSpec` covers JSON decode and round-trip behavior for the first model slice, commit-status codecs, notification codecs, enum validation, pagination codec behavior, auth modes, and the error ADT.
...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, and pull-request merge/update endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, request-body presence, retryability, query enum values including `repoUpdatePullRequest` style values, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups. Documented non-2xx response status/ref comparison is currently complete for the pull-request merge/update endpoints and should be expanded to the other audited endpoint groups before their contracts are described as fully hardened.
...
- Fresh senior review of the bounded contract-hardening gate found no functional blocker. Residual follow-up remains around generic mapper coverage for global 405/423 behavior, avoiding unnecessary public API exposure for audit-only metadata, and expanding documented non-2xx response-label checks beyond the merge/update endpoint group.
...
- before adding a large new surface, add mapper-level tests for the global `405` and `423` classifications, including empty-body or non-JSON bodies where Swagger documents `#/responses/empty`, so the behavior is not only covered through pull-request merge/update tests,
- decide whether `GiteaEndpoint.nonSuccessResponses` and `GiteaResponseLabel` are intended public API; if they are audit-only metadata, move or restrict them before the first release rather than letting test-support shape leak into the client API snapshot,
- expand documented non-2xx response status/ref label checks for the already audited pull-review lifecycle and commit-status endpoint groups, or narrow the documentation to avoid overclaiming hardened coverage,
- add the next Swagger-backed pull-request write slice that complements merge/update, likely pull-request creation/editing or another small lifecycle command group from `plugin-redoc-2.yaml`,
- start new write slices with request-layer tests and endpoint metadata audit coverage for operation ID, method, path, required path parameters, success response labels, request-body presence, retryability, query enum values where documented, and documented non-2xx response status/ref labels,
- preserve the established no-body lifecycle command conventions: shared no-body builder, no `Content-Type`, non-retryable write semantics, explicit empty-response decoding, and documented failure tests,
- keep write facade methods small and typed, with `BackendStub` facade tests for success, representative errors, and retry behavior, including `GiteaError.MethodNotAllowed` and `GiteaError.Locked` where the Swagger contract documents 405/423 responses,
