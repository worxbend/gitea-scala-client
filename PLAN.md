...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, and single-commit endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, single-commit boolean query toggles, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single repository commit lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull and single-commit GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- implement the adjacent Swagger-backed commit diff/patch slice, `GET /repos/{owner}/{repo}/git/commits/{sha}.{diffType}` (`repoDownloadCommitDiffOrPatch`), returning the raw diff or patch string,
- add a typed diff-type value for the documented `diff` and `patch` path values, likely a commit-specific enum unless the local API shape clearly favors reusing an existing diff-type model,
- add endpoint metadata with method, path, operation ID, required owner/repo/sha/diffType path parameters, success response label `#/responses/string`, no request body, read-only retry eligibility, and private audit expectations for documented `404` not found response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, safe `diffType` path rendering, `Accept: text/plain`, shared auth/OTP/user-agent headers, raw string response decoding, documented not-found propagation, and read-only retry semantics,
- expose a small typed facade method on `ReposApi`, likely `commitDiffOrPatch(owner: String, repo: String, sha: String, diffType: CommitDiffType): IO[GiteaError, String]` unless nearby naming suggests a better fit,
- add request-layer tests for method/path/header behavior, safe path encoding, text/plain acceptance, raw success decoding, documented `404` error mapping, and retryability, plus facade tests for success, failure propagation, and retry behavior through `BackendStub`,
- extend `GiteaEndpointAuditSpec` without adding audit-only fields back to published endpoint metadata,
- preserve the newly added optional-query-parameter audit for the commit endpoints, and make the diff/patch audit prove there are no undocumented query or body parameters on `repoDownloadCommitDiffOrPatch`,
- decide the repository-commit facade naming while this surface is still small: keep `ReposApi.commit` only if the adjacent diff/patch method reads naturally beside it, otherwise rename before the next snapshot refresh to avoid avoidable public API churn,
- keep the generic `PreconditionFailed` taxonomy in place; only add more explicit non-5xx error cases when Swagger introduces distinct semantics that callers can act on,
- refresh public API snapshots after intentional signatures compile, then run `./mill core.test`, `./mill client.test`, and `./mill compatibility.check`; run `./mill __.test`, `./mill it.test`, and `./mill examples.run` when time permits and keep them hermetic without live credentials,
