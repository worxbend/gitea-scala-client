...
- `GiteaClientSpec` also covers successful combined commit-status lookup, commit-status list streaming by ref and SHA, commit-status creation, transport/decode error propagation, and non-retryable POST semantics through a `BackendStub[Task]`.
...
- extend the endpoint metadata audit to the newly added commit-status endpoints before adding more repository write surface; cover operation ID, method, path, required path parameters, success response labels, request-body presence, retryability, and the implemented `sort`/`state` query enum values,
- decide whether `GET /repos/{owner}/{repo}/commits/{ref}/status` needs public combined-status pagination controls, because the low-level request builder accepts `page`/`limit` while the current facade only fetches the first combined-status page,
- continue with pull-request merge/write operations from `plugin-redoc-2.yaml`, prioritizing the smallest Swagger-backed set that adds practical PR workflow coverage beyond merge-status checks and review lifecycle commands,
- preserve no-body lifecycle command coverage with explicit request-body, content-type, retryability, and documented failure assertions when adding more lifecycle commands,
- keep commit-status enum coverage explicit: response/request payloads support `skipped`, while commit-status list query filters currently do not expose `skipped` because the Swagger query enum omits it,
