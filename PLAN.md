...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, and commit-to-pull-request endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- implement the next Swagger-backed commit read slice, starting with `GET /repos/{owner}/{repo}/git/commits/{sha}` (`repoGetSingleCommit`) returning a single `Commit`,
- add a small typed params value, likely `SingleCommitParams`, for the documented optional `stat`, `verification`, and `files` boolean query parameters, preserving Swagger names and default omission behavior,
- add endpoint metadata with method, path, operation ID, required owner/repo/sha path parameters, optional `stat`, `verification`, and `files` query parameters, success response label, no request body, read-only retry eligibility, and private audit expectations for documented `404` not found and `422` validation-error response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, `Accept: application/json`, shared auth/OTP/user-agent headers, optional `stat`, `verification`, and `files` query encoding, successful `Commit` decoding, documented not-found/validation-error propagation, and read-only retry semantics,
- expose a small typed facade method on the most coherent existing API surface, likely `ReposApi.commit(owner: String, repo: String, sha: String): IO[GiteaError, Commit]` unless local naming suggests a better fit,
- add request-layer tests for method/path/header behavior, safe encoding, query omission and explicit boolean query encoding, success decoding, documented `404`/`422` error mapping, and retryability, plus facade tests for success, failure propagation, and retry behavior through `BackendStub`,
