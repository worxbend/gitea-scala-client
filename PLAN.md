...
- likely continue with another small issue-management slice from `plugin-redoc-2.yaml`, such as `POST/DELETE /repos/{owner}/{repo}/issues/{index}/pin` (`pinIssue`, `unpinIssue`) and `PATCH /repos/{owner}/{repo}/issues/{index}/pin/{position}` (`moveIssuePin`),
