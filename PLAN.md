...
- Highest-value next continuation: implement the adjacent read-only repository team visibility slice only after a quick Swagger model check. Candidate endpoints are `GET /repos/{owner}/{repo}/teams` (`repoListTeams`, `#/responses/TeamList`) and `GET /repos/{owner}/{repo}/teams/{team}` (`repoCheckTeam`, `#/responses/Team`). Keep the slice bounded to read-only team listing/checking, model only the Swagger response fields needed for `Team`, avoid team add/delete/write operations, and register request/facade/audit/docs/snapshot coverage from the start.
- If the repository team response model drags in broad organization/admin shape or write-only request payloads, choose another narrow `GET`-only repository endpoint group with existing models and clear pagination semantics instead of forcing a large model slice.
