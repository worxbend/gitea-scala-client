...
- `PullRequestsApi` is mixed into `GiteaClient` with unambiguous `client.pullRequests(owner, repo, params)`, `client.pinnedPullRequests(owner, repo)`, `client.pullRequestByBaseHead(owner, repo, base, head)`, `client.pullRequest(owner, repo, index)`, and `client.pullRequestFiles(owner, repo, index, params)` facade methods.
- Pull request list responses decode through the existing paginated JSON mapper as `#/responses/PullRequestList`; pinned pull-request lists decode as non-paginated chunks from `#/responses/PullRequestList`; single pull request and base/head lookup responses decode as `#/responses/PullRequest`; changed-file responses decode through the existing paginated JSON mapper as `#/responses/ChangedFileList`.
...
- likely continue with the next pull-request read slice from `plugin-redoc-2.yaml`, such as `GET /repos/{owner}/{repo}/pulls/{index}/commits` (`repoGetPullRequestCommits`), or move to the next small repository-management endpoint,
