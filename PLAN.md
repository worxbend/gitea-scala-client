...
- add a hermetic-by-default live integration probe for slash-containing Git refs, gated on explicit repository data such as `GITEA_OWNER`, `GITEA_REPO`, and `GITEA_REF` with a default example of `heads/main`; keep it ignored unless all live variables are present and do not block normal `it.test`,
- consider a second opt-in live probe for annotated tags only if an annotated tag SHA can be supplied explicitly, for example through `GITEA_ANNOTATED_TAG_SHA`; do not assume repository tag-list entries are annotated tag objects,
- choose the next small Swagger-backed repository read or metadata slice from `plugin-redoc-2.yaml`; before coding, verify whether it introduces a new response shape, path value, query enum, or body-pagination contract so the audit/test plan covers that shape from the start,
- add a lightweight schema-field checklist for new Swagger response models, either as test comments or a small test helper, that records the referenced Swagger definition name and expected field list; endpoint audits catch operation drift but not missing model fields,
