...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, commit note, Git tree, Git blob, Git refs, repository contents, raw/media repository files, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, path enum values including commit and pull-request `diffType` values, single-commit and commit-note boolean query toggles, Git tree `recursive`/`page`/`per_page` query toggles, repository contents/raw/media `ref` query handling, raw/media `application/octet-stream` Swagger `type: file` response shape, Git blob and Git refs no-query/no-body checks, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit note, Git tree, Git blob, Git refs, repository contents metadata, raw/media repository byte downloads, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, raw byte decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- Thirty-ninth typed repository archive download slice is implemented but requires cleanup before it should be treated as release-ready. The working code adds `GET /repos/{owner}/{repo}/archive/{archive}` (`repoGetArchive`) with safe owner/repo/archive path encoding, slash-containing and dot-containing archive values such as `refs/heads/main.tar.gz`, `main.zip`, and `v1.0.0.tar.gz` encoded as one `archive` path segment, `Accept: application/octet-stream`, no request body, no JSON `Content-Type`, successful response decoding as buffered `Chunk[Byte]`, documented `404` propagation, read-only retry eligibility, `ReposApi.archive(owner, repo, archive)`, request-layer tests, facade and retry tests, Swagger-backed endpoint audit coverage, docs, and public API snapshot updates. Fresh review found that the local Swagger operation also documents an optional multi-value `path` query parameter for repository subpaths; the current public API cannot send it, and README/CHANGELOG/PLAN text incorrectly describes the endpoint as having no query parameters.
- Live archive download probe is complete in `LiveGiteaIntegrationSpec`, gated on `GITEA_ARCHIVE` plus `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, and `GITEA_REPO`. The read-only probe calls `ReposApi.archive(owner, repo, archive)`, asserts a non-empty `Chunk[Byte]`, and remains ignored during normal credential-free `it.test` runs.
- Documentation and public API snapshot alignment for the archive download API is incomplete. The snapshots currently include unplanned repository write API additions (`CreateRepo`, `ForkRepo`, `ReposApi.create`, `ReposApi.fork`, `ReposApi.delete`, and related request builders/endpoints) that were not part of the archive iteration and have no request/client/audit/docs coverage. Validation of the current checkout passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, focused `client.test.testOnly` for request/audit/facade specs, `./mill --no-server __.compile`, and credential-stripped `it.test`; passing validation does not make the accidental public ABI additions acceptable.
- Fresh senior review of the archive download slice found no compile/test blocker, but identified release-blocking cleanup items: remove or deliberately complete the unplanned repository create/fork/delete surface before committing snapshots, add or explicitly defer support for the documented archive `path` query parameter, and correct docs/audit wording that claims archive success is Swagger `type: file` / `application/octet-stream` even though `plugin-redoc-2.yaml` records `produces: application/json` with a bare `200` success description.

## Next Continuation

The next continuation should stabilize the archive slice before expanding endpoint coverage:

- first, quarantine the unplanned repository write surface: either remove `CreateRepo`, `ForkRepo`, `createCurrentUserRepo`, `createFork`, `repoDelete`, `ReposApi.create`, `ReposApi.fork`, `ReposApi.delete`, their request builders, and their snapshot entries from this archive slice, or complete them as a deliberate write slice with schema-complete models, request/client/audit tests, docs, changelog, plan updates, and compatibility snapshots,
- add an `ArchiveParams` or equivalent typed request parameter for the documented optional multi-value `path` query parameter on `repoGetArchive`, or explicitly document that `ReposApi.archive(owner, repo, archive)` is a whole-archive-only convenience while a follow-up will expose subpath archives; the endpoint audit and request tests must stop claiming the Swagger operation has no query parameters,
- correct README, CHANGELOG, PLAN, and `GiteaEndpointAuditSpec` wording for archive downloads: the client may choose `Accept: application/octet-stream` and decode bytes, but the local Swagger contract records `produces: application/json` and a bare `200` success description, not a `type: file` / `application/octet-stream` schema,
- decide whether the low-level `ArchiveFormat` overload is useful public API; if kept, add core/request tests and consider exposing it coherently through the facade, otherwise remove it from `core` and the API snapshot to avoid an unexercised ABI addition,
- after the archive cleanup passes review, add the repository release asset endpoints (`GET /repos/{owner}/{repo}/releases/{id}/assets`, `GET /repos/{owner}/{repo}/releases/assets/{id}`, and the documented asset download path if present in `plugin-redoc-2.yaml`) as the next common binary download use case, using `application/octet-stream` + `Chunk[Byte]` through the stabilized byte response executor path,
- alternatively, after cleanup, choose another narrowly scoped repository read endpoint group that has simple path/query semantics and can be audited end to end against `plugin-redoc-2.yaml`,
...
- keep `ReposApi.rawFile` and `ReposApi.mediaFile` as buffered `Chunk[Byte]` for Swagger `type: file` endpoints, and keep `ReposApi.archive` buffered as a pragmatic byte-download facade unless live behavior or future Swagger drift justifies a streaming design; defer streaming download abstractions to a deliberate large-binary slice,
