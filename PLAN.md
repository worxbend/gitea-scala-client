...
- Core supporting types now include `Page`, `Auth`, and the `GiteaError` ADT, including explicit `MethodNotAllowed` and `Locked` resource-state cases for documented 405/423 responses.
...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, and pull-request merge/update endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence, retryability, query enum values including `repoUpdatePullRequest` style values, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaResponseMapper` decodes successful JSON responses, non-paginated JSON arrays, paginated issue/repository/branch/tag/release/pull-request/review/reaction/commit-status/notification lists, object-shaped user-search and topic-name pages, repository pin-capacity responses, 204/unit responses, pull-request merge-status 204/404 booleans, Gitea error payloads, raw failure bodies, explicit 405 `MethodNotAllowed` and 423 `Locked` failures, pagination headers, and rate-limit reset headers. Mapper-level tests cover global 405/423 classification for JSON error payloads, empty bodies, and non-JSON bodies.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- `GiteaRequestsSpec` covers release, commit status, pull request including pinned pull-request listing, base/head lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/locked mapping.
...
- first tighten non-5xx error taxonomy by adding an explicit `GiteaError.PreconditionFailed` or similarly named case for documented `412` responses, mapping it in `GiteaResponseMapper`, covering JSON/empty/raw-body responses at mapper level, updating pull-request edit request/facade tests to expect the typed error, and refreshing public API snapshots and docs,
- implement the next Swagger-backed repository/pull-request read slice for `GET /repos/{owner}/{repo}/commits/{sha}/pull` (`repoGetCommitPullRequest`) returning the merged pull request associated with a commit SHA,
- add pure request construction with safe owner/repo/sha path encoding, `Accept: application/json`, shared auth/OTP/user-agent headers, successful `PullRequest` decoding, and read-only retry eligibility,
- add request-layer and facade tests for success, path encoding, auth/OTP/user-agent behavior, retry behavior for this GET endpoint, and documented `404` mapping,
- add endpoint metadata audit coverage for `repoGetCommitPullRequest`: operation ID, method, path, required path parameters, success response label, request-body absence, retryability, and documented non-2xx response status/ref labels,
- keep audit-only response-label expectations test-private; if future audit groups need shared structure, refactor `GiteaEndpointAuditSpec` rather than extending published `GiteaEndpoint` metadata,
- keep the facade method small and typed, preferably on `PullRequestsApi` unless the local API shape clearly fits `ReposApi` better,
- refresh public API snapshots after intentional public signatures compile, then run `./mill core.test`, `./mill client.test`, and `./mill compatibility.check`; run `./mill __.test`, `./mill it.test`, and `./mill examples.run` when time permits and keep them hermetic without live credentials,
- keep README, CHANGELOG, and this PLAN aligned with any newly implemented API surface and validation results.
