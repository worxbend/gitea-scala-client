...
- Keep slash-containing release-tag routing pending live observation. Run the opt-in `client.releaseByTag(owner, repo, tag)` probe only when a real repository has a known slash-containing release tag such as `release/candidate`; record the exact configured tag and accept/reject behavior in README, CHANGELOG, and this plan only after that enabled run succeeds or fails against real Gitea. Normal tags such as `v1.0.0` remain generic release-by-tag confidence only.
- If no suitable live collaborator/team/release-tag fixtures are available, avoid another documentation-only cycle and choose the next bounded read-only Swagger slice with a clear contract. A good candidate is `GET /repos/{owner}/{repo}/tags/{tag}` (`repoGetTag`) because the `Tag` model already exists, the endpoint is path-only and read-only, and it would let tag-name path encoding be tested separately from release-by-tag routing.
