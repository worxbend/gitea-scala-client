...
- Client HTTP also has a reusable Swagger-backed endpoint metadata audit guardrail for pull-request review lifecycle, commit-status, pull-request create/edit, pull-request merge/update, commit-to-pull-request, single-commit, and commit diff/patch endpoints against `plugin-redoc-2.yaml`, covering operation IDs, methods, paths, required path parameters, success response labels, documented non-2xx response status/ref labels, request-body presence or absence, retryability, query enum values including `repoUpdatePullRequest` style values, single-commit boolean query toggles, commit diff/patch no-query/no-body checks, no-body lifecycle POST content-type behavior, and loud assertion messages for missing path/method/parameter lookups.
...
- `GiteaClientSpec` also covers multi-page pull request, changed-file, and commit streaming, pinned pull-request listing, base/head pull-request lookup, commit-to-pull-request lookup, single repository commit lookup, commit diff/patch raw text lookup, single-pull-request lookup, pull-request create/edit success and representative error handling, retryable commit-to-pull, single-commit, and commit diff/patch GET behavior, and non-retryable create/edit write semantics through a `BackendStub[Task]`.
...
- `GiteaRequestsSpec` covers release, commit status, single repository commit, commit diff/patch raw text downloads, pull request including pinned pull-request listing, base/head lookup, commit-to-pull-request lookup, create/edit, reviews, review-request creation/cancellation, review creation/submission/dismissal/undismissal, review-comment resolution/unresolution, merge/update commands, changed files, and commits, notification, issue-create/edit/delete/pin/pinned-list/comment/deadline/dependency/blocking/reaction/subscription/tracked-time/stopwatch, repository pin-capacity checks, current-user stopwatch listing, issue-comment listing/lookup/editing/deletion/reaction, issue-label, and issue-lock endpoint metadata, path encoding, page/limit and filter query parameters, JSON body construction, JSON decoding, empty response decoding, and not-found/method-not-allowed/precondition-failed/locked mapping.
...
- first harden `GiteaEndpointAuditSpec` so it can compare path enum values against Swagger, then register checks for `repoDownloadCommitDiffOrPatch` `diffType` and the existing pull-request diff/patch `diffType` values without moving audit-only data into public endpoint metadata,
- implement the adjacent Swagger-backed commit note read slice, `GET /repos/{owner}/{repo}/git/notes/{sha}` (`repoGetNote`), returning the documented `Note` JSON response,
- add the minimal typed `Note` model and codecs from `plugin-redoc-2.yaml`, reusing existing commit/user submodels only when the schema matches exactly,
- add a small params model for the documented optional `verification` and `files` boolean query toggles, mirroring the `SingleCommitParams` style without introducing shared abstraction unless duplication becomes meaningful,
- add endpoint metadata with method, path, operation ID, required owner/repo/sha path parameters, success response label `#/responses/Note`, no request body, read-only retry eligibility, and private audit expectations for documented `404` and `422` response status/ref labels,
- add a pure request builder with safe owner/repo/sha path encoding, JSON accept headers, shared auth/OTP/user-agent headers, optional query encoding, `Note` response decoding, documented failure propagation, and read-only retry semantics,
- expose a typed facade method on `ReposApi` with an explicit name such as `commitNote(owner: String, repo: String, sha: String, params: CommitNoteParams): IO[GiteaError, Note]` if that naming fits the surrounding repository-commit API,
- add focused core/request/facade tests for codec behavior, method/path/query/header construction, success decoding, documented 404/422 mapping, and retry behavior through `BackendStub`,
- extend `GiteaEndpointAuditSpec` without adding audit-only fields back to published endpoint metadata, and keep optional query parameter audits explicit for repository-commit endpoints,
