...
- If no live fixtures are available, avoid another evidence-only documentation loop and proceed with a small read-only Swagger fallback after confirming the exact local contract. A good current candidate is `GET /repos/{owner}/{repo}/assignees` (`repoGetAssignees`): the local Swagger confirms it is a path-only read with no query parameters, no request body, `#/responses/UserList` success, and documented `404`; it can reuse the existing `User` model and non-paginated chunk decoding while avoiding repository write, hook, key, search, and admin/action surfaces.
