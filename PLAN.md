...
- Release-by-tag live confidence is complete for the existing read-only `client.releaseByTag(owner, repo, tag)` facade. `LiveGiteaIntegrationSpec` has an opt-in probe gated on all five required variables being non-empty: `GITEA_URL`, `GITEA_TOKEN`, `GITEA_OWNER`, `GITEA_REPO`, and `GITEA_RELEASE_TAG`. When enabled, the probe builds the live ZIO backend, passes the configured tag through unchanged to `client.releaseByTag(owner, repo, tag)`, and asserts that the returned `Release.tagName` contains exactly the configured `GITEA_RELEASE_TAG`; slash-containing values such as `release/candidate` are the intended confidence case. With credentials and endpoint-specific variables absent, default credential-free `it.test` remains hermetic, reports the probe ignored, and makes no network call.
- Fresh senior review of the release-by-tag live-probe documentation slice found no functional blocker. The actual release-by-tag probe existed from the earlier release-confidence work; this iteration primarily aligned README, CHANGELOG, and the continuation plan around using `GITEA_RELEASE_TAG` as the live assertion variable for slash-bearing tag routing. Focused validation passed with `git diff --check`, `./mill --no-server core.test client.test compatibility.check`, and credential-stripped `env -u GITEA_URL -u GITEA_TOKEN -u GITEA_USERNAME -u GITEA_PASSWORD -u GITEA_OWNER -u GITEA_REPO -u GITEA_REF -u GITEA_ANNOTATED_TAG_SHA -u GITEA_CONTENTS_FILEPATH -u GITEA_CONTENTS_REF -u GITEA_RAW_FILEPATH -u GITEA_RAW_REF -u GITEA_ARCHIVE -u GITEA_ARCHIVE_PATHS -u GITEA_RELEASE_ID -u GITEA_RELEASE_TAG -u GITEA_LATEST_RELEASE_TAG -u GITEA_RELEASE_ASSET_ID -u GITEA_ORG -u GITEA_USER_QUERY -u GITEA_PAGE_SIZE -u GITEA_TIMEOUT -u GITEA_MAX_RETRIES ./mill --no-server it.test`; all twelve live probes were ignored without credentials. Residual risk: the credential-stripped validation proves hermetic skipping only, not real slash-containing release-tag routing.

## Next Continuation

- Next low-risk continuation: run the opt-in `client.releaseByTag(owner, repo, tag)` probe against a real Gitea repository only when a known slash-containing release tag such as `release/candidate` is available. Record the observed accept/reject behavior narrowly as `repoGetReleaseByTag` evidence in README, CHANGELOG, and this plan. If no such live repository state is available, leave slash-tag routing marked as pending observation rather than claiming it was verified.
- If the release-by-tag probe is used only with a normal tag such as `v1.0.0`, treat the result as generic release-by-tag live confidence, not slash-routing confidence. Consider a separate documented assertion variable only if ordinary and slash-bearing release-tag probes need to coexist for the same repository.
