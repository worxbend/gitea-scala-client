...
object GiteaEndpoints:
  ...
  val repoGetLatestRelease: GiteaEndpoint =
    GiteaEndpoint(
      method = "GET",
  ...
  /** Every endpoint declared above, in declaration order.
    *
    * The audit suite folds over this list and checks each entry against the OpenAPI
    * contract, so an endpoint that is missing here is an endpoint nothing verifies.
